...
description: How pi and OMP use the authenticated local llama.cpp router, how the setup engine installs and launches pinned agents, and how complete model tiers become OMP role mappings. Covers preservation boundaries, remote sessions, optional Herdr coordination, and explicit upgrades.
...
The interactive manager uses the same sequence: validate or install the prospective agent, save `AGENT`, then invoke `agent`. `agent-upgrade [pi|omp]` is intentionally separate. It replaces the named agent with the configured pinned version, validates its reported version, and refreshes only integration that belongs to the currently selected agent.
...
The defaults are `PI_VERSION=0.84.4` and `OMP_VERSION=18.0.10`. Both are persisted configuration keys and must be exact semantic versions. New installs and upgrades verify that the expected executable exists and reports the selected version. The OMP configuration schema is additionally version-gated: if an OMP executable is installed, `routing` refuses to write its v18 routing for a version other than `OMP_VERSION`. This makes upgrading a deliberate compatibility review, rather than a side effect of ordinary setup. A direct OMP installation remains useful before downloading a model because it can establish the binary, shell integration, and selected-agent launcher, while warning that role routing needs a complete tier.
...
The resolver never selects an absent alias. It selects **base** as the first complete tier in `everyday → coder → senior` order; **specialist** is complete `coder` or base; **architect** is complete `senior` or base. With no complete tier, routing fails before writing active files. `sticky` maps all roles to base; `balanced` promotes only `task` to specialist; and `quality` additionally maps `slow`, `plan`, `advisor`, and `designer` to architect. The fixed base roles are `default`, `vision`, `smol`, `commit`, `tiny`, and `title`.

```mermaid
flowchart TD
  Tiers["Complete locked tiers"] --> Base{"First complete tier"}
  Base -->|"everyday"| Everyday["qwen3.8-27b"]
  Base -->|"otherwise coder"| CoderBase["coder"]
  Base -->|"otherwise senior"| SeniorBase["qwen3.5-122b-a10b"]
  Base -->|"none"| Blocked["Refuse routing"]
  Tiers --> Specialist{"Coder complete"}
  Specialist -->|"yes"| Coder["specialist coder"]
  Specialist -->|"no"| SpecialistBase["specialist base"]
  Tiers --> Architect{"Senior complete"}
  Architect -->|"yes"| Senior["architect senior"]
  Architect -->|"no"| ArchitectBase["architect base"]
  Everyday --> Roles["Resolve OMP model roles"]
  CoderBase --> Roles
  SeniorBase --> Roles
  Coder --> Roles
  SpecialistBase --> Roles
  Senior --> Roles
  ArchitectBase --> Roles
  Roles --> Sticky["sticky all base"]
  Roles --> Balanced["balanced task specialist"]
  Roles --> Quality["quality task specialist and review architect"]
  Sticky --> Router["Authenticated local llama.cpp router"]
  Balanced --> Router
  Quality --> Router
```

*Complete-tier fallback determines base, specialist, and architect before the routing profile assigns OMP roles to the authenticated local router.*
...
## Optional LSP, remote sessions, and Herdr coordination
...
Herdr is an optional, separate coordination layer (`HERDR_ENABLED=1` by default), not part of OMP routing. `herdr-config` requires distinct OMP and pi agent directories, writes the coordination skill to both, and installs a per-agent lifecycle hook only when its prior hook and receipt still prove managed ownership; custom or modified hooks and unsafe symlink paths are preserved. The hooks are inert outside a Herdr pane. When `HERDR_ENV=1`, a socket path, and a pane ID are present, they send session and `working`/`blocked`/`idle` reports to Herdr’s local RPC endpoint and include an absolute session path when available, otherwise a session ID. Delivery is best-effort: a short request attempt is retried once, so a reporting failure does not make the coding agent unavailable.

The pi hook reports only TUI sessions, avoiding headless RPC/print modes. The OMP hook additionally debounces idle reporting and holds `working` during a configurable grace period for retryable provider failures before it reports `blocked`; it also treats tool approval and `ask` prompts as blocked. These are observability semantics for persistent workspaces, not changes to the model-routing or credential contract.
...
Focused regression coverage is behavioral. `tests/unit/agent-isolation-test.sh` proves the Omarchy private-prefix boundary, confirms launchers and remote sessions do not execute moving stubs, checks fresh per-agent directory/key propagation, and verifies fail-closed behavior when the private runtime disappears. `tests/integration/generated-config-test.sh` covers zero-tier rejection, direct OMP setup before a model download, managed-export replacement, shell-symlink safety, session naming, routing metadata/profile fallbacks, custom-pair protection, and rollback. `tests/e2e/workflow-test.sh` verifies an applied managed routing set is refreshed after tier loss even when `AGENT=pi`.
